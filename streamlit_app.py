...
import requests
from dags.ml_pipeline import MLPipeline
...
def load_from_mlflow(
    tracking_uri: str, experiment_name: str, model_name: str = "customer_churn_model"
):
    ...
    try:
        ...
        try:
            ...
            st.info(
                f"   Loading version {latest_version.version} from run {registry_run_id}"
            )
            ...
            response = requests.get(
                list_url, params={"run_id": registry_run_id, "path": "best_model"}
            )
            ...
            model_pkl = next(
                (f for f in files if f["path"].endswith("_model.pkl")), None
            )
            transformer_pkl = next(
                (
                    f
                    for f in files
                    if "transformer" in f["path"].lower() and f["path"].endswith(".pkl")
                ),
                None,
            )
            ...
            # Download model
            st.info(
                f"   Downloading model ({model_pkl.get('file_size', 0) // 1024 // 1024}MB)..."
            )
...
def prepare_features(
    pipeline: MLPipeline, model: Any, selected_features: List[str], df: pd.DataFrame
) -> pd.DataFrame:
    feature_set = pipeline.engineer_features(df, fit=False, target_col=None)
    feats = feature_set.features
    used_cols = [
        c for c in selected_features if c in feats.columns
    ] or feats.columns.tolist()
...
with st.sidebar:
    ...
    tracking_uri = st.text_input(
        "MLflow Tracking URI",
        value=os.getenv("MLFLOW_TRACKING_URI", "http://localhost:5001"),
    )
    model_name = st.text_input(
        "Model Name", value=os.getenv("MLFLOW_MODEL_NAME", "customer_churn_model")
    )

    st.divider()
    col1, col2 = st.columns(2)
    load_button = col1.button(
        "🔄 Load Latest Model", type="primary", use_container_width=True
    )
...
if load_button:
    with st.spinner("🔄 Loading latest model from MLflow..."):
        ...
        if m:
            ...
            st.error(
                "❌ Failed to load model from MLflow. Check the debug panel for details."
            )
...
if not model or pipeline is None:
    st.warning(
        "⚠️ Model not loaded. Click '🔄 Load Latest Model' in the sidebar to get started."
    )
else:
    ...
    with tab_single:
        ...
        WarehouseToHome = cols1[2].number_input(
            "WarehouseToHome", min_value=0.0, value=10.0
        )

        HourSpendOnApp = cols2[0].number_input(
            "HourSpendOnApp", min_value=0.0, value=2.0
        )
        NumberOfDeviceRegistered = cols2[1].number_input(
            "NumberOfDeviceRegistered", min_value=0, value=2
        )
        SatisfactionScore = cols2[2].number_input(
            "SatisfactionScore", min_value=1.0, max_value=5.0, value=3.0
        )
        ...
        OrderAmountHikeFromlastYear = cols3[2].number_input(
            "OrderAmountHikeFromlastYear", value=0.0
        )
        ...
        DaySinceLastOrder = cols4[2].number_input(
            "DaySinceLastOrder", min_value=0, value=7
        )

        cols5 = st.columns(3)
        CashbackAmount = cols5[0].number_input(
            "CashbackAmount", min_value=0.0, value=0.0
        )
        PreferredLoginDevice = cols5[1].selectbox(
            "PreferredLoginDevice", CATEGORICAL_DEFAULTS["PreferredLoginDevice"]
        )
        PreferredPaymentMode = cols5[2].selectbox(
            "PreferredPaymentMode", CATEGORICAL_DEFAULTS["PreferredPaymentMode"]
        )

        cols6 = st.columns(3)
        Gender = cols6[0].selectbox("Gender", CATEGORICAL_DEFAULTS["Gender"])
        PreferedOrderCat = cols6[1].selectbox(
            "PreferedOrderCat", CATEGORICAL_DEFAULTS["PreferedOrderCat"]
        )
        MaritalStatus = cols6[2].selectbox(
            "MaritalStatus", CATEGORICAL_DEFAULTS["MaritalStatus"]
        )
        ...
        if st.button("Predict", type="primary"):
            try:
                ...
                if seg == "high":
                    ...
                    st.warning(
                        "📊 **Medium Risk Customer** - Monitor and engage proactively"
                    )
                else:
    ...
    with tab_batch:
        ...
        st.caption(
            "Upload a file with customer data. Required columns: "
            + ", ".join([c for c in RAW_COLUMNS if c != "CustomerID"][:5])
            + "..."
        )
        ...
        if uploaded is not None:
            try:
                ...
                missing = [
                    c for c in RAW_COLUMNS if c not in df.columns and c != "CustomerID"
                ]
                if missing:
                    ...
                    with st.spinner("Making predictions..."):
                        ...
                        out = pd.DataFrame(
                            {
                                "CustomerID": df.get(
                                    "CustomerID", pd.Series(range(len(df)))
                                ).values,
                    ...
                    col1.metric(
                        "High Risk",
                        f"{res['high_risk_count']} ({res['high_risk_count']/len(out):.1%})",
                    )
                    col2.metric(
                        "Medium Risk",
                        f"{res['medium_risk_count']} ({res['medium_risk_count']/len(out):.1%})",
                    )
                    col3.metric(
                        "Low Risk",
                        f"{res['low_risk_count']} ({res['low_risk_count']/len(out):.1%})",
                    )
